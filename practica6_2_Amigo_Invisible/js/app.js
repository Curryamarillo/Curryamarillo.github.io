...
function reparto(){
    ...
    for (let i = 0; i < amigos.length; i++) {
        let dador = amigos[i]
        let opciones = quien.filter(nombre => nombre !== dador)
        if (opciones.length === 0) {
            esCorrecto = false;
            break
        }
        let indexSorteo = aleatorio(0, opciones.length - 1)
        let elegido = opciones[indexSorteo]
        solucionClientes.set(dador, elegido)
        let indexQuitar = quien.indexOf(elegido)
        quien.splice(indexQuitar, 1)
    }
